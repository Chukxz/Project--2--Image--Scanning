if __name__ != "__main__":
    ...
    intermediate_dir =  path.join('Edge_Detection_V1.0','Resources','Generated_Images')
    ...
    def createchildfile(file_name,ext,dir_name=''):
        ...
        if dir_name != '':
            ...
            dir_path = path.join(intermediate_dir,dir_name,new_file)
        else:
            dir_path = path.join(intermediate_dir,new_file)
    ...
    def deletechildfile(file_name,ext,dir_name=''):
        ...
        if dir_name !='':
            dir_path = path.join(intermediate_dir,dir_name,delete_file)
        else:
            dir_path = path.join(intermediate_dir,delete_file)    
        try:
    ...
    def copyimagefile(file_name,ext):
        ...
        src_file_path = path.join('Edge_Detection_V1.0','Resources','Images',full_file_name)
        resized_img_path = path.join('Edge_Detection_V1.0','Resources','Images',resized_img_file_name)
        dest_folder_path = path.join(intermediate_dir,file_name)
        ...
        try:
            if path.exists(src_file_path):
                ...
                if not path.exists(dest_file_path):
                    #Resize Image to 480 X 480 pixels to reduce space
                    img = Image.open(src_file_path)
                    width, height = img.size

                    conversion_factor = max(width,height)/480
                    with open(path.join(dest_folder_path,"conv_factor.txt"),"w") as conv_f:
                    ...
                    copy(resized_img_path,dest_file_path)
